# black9.py
# Complete code with all necessary functions including leader extraction
...
def openroom(K, V):
    fields = {
        1: 2,
        2: {
            1: 1, 2: 15, 3: 5, 4: "[B][00FF00]SHAWON SIR", 5: "1", 6: 12, 7: 1, 8: 1, 9: 1, 11: 1, 12: 2, 14: 36981056,
            15: {1: "IDC3", 2: 126, 3: "ME"}, 16: "\u0001\u0003\u0004\u0007\t\n\u000b\u0012\u000f\u000e\u0016\u0019\u001a \u001d",
...
def SPMROOMZIX(Uid, Rm, Nm, K, V):
    fields = {
        1: 2,
        2: {
            1: 1, 2: 15, 3: 5, 4: Nm, 5: Rm, 6: 12, 7: 1, 8: 1, 9: 1, 11: 1, 12: 2, 14: int(Uid),
            15: {1: "IDC3", 2: 126, 3: "ME"},
            16: "\u0001\u0003\u0004\u0007\t\n\u000b\u0012\u000f\u000e\u0016\u0019\u001a \u001d",
            18: int(Rm), 27: 1, 34: "\u0000\u0001", 40: "en", 48: 1, 49: {1: 21},
...
def CHangeRoomName(room_id, key, iv):
    ...
    packet = CrEaTe_ProTo(fields).hex()
    encrypted_packet = EnC_PacKeT(packet, key, iv)
    hlen = len(encrypted_packet) // 2
    return bytes.fromhex("0E15000000" + DecodE_HeX(hlen) + encrypted_packet)
...
def SPamSq(Uid, K, V): 
    fields = {
        1: 33,
        2: {
            1: int(Uid), 2: 'ME', 3: 1, 4: 1, 7: 330, 8: 19459, 9: 100, 12: 1, 16: 1,
            17: {2: 94, 6: 11, 8: '1.111.5', 9: 3, 10: 2}, 18: 201, 23: {2: 1, 3: 1},
            24: xBunnEr(), 26: {}, 28: {}
        }
    }
    return GeneRaTePk(str(CrEaTe_ProTo(fields).hex()), '0515', K, V)
...
def Zix(self, player_id, key, iv):
    fields = {1: 5, 2: {1: int(player_id), 2: 1, 3: int(player_id), 4: """[b][c][FF0000]▒█▀▀█ ░█▀█░ 　"""}}
    return GeneRaTePk(str(CrEaTe_ProTo(fields).hex()), '0515', key, iv)

def Zix1(self, player_id, key, iv):
    fields = {1: int(player_id), 2: 5, 4: 50, 5: {1: int(player_id), 2: "[00FF00]SHAWON SIR BOT V1", 3: 1}}
    return GeneRaTePk(str(CrEaTe_ProTo(fields).hex()), '0515', key, iv)
...
def ghost_pakcet(player_id, nm, secret_code, key, iv):
    fields = {
        ...
        2: {
            1: int(player_id),
            2: {1: int(player_id), 2: 1159, 3: f"[b][c][{ArA_CoLor()}]{nm}", 5: 22, 6: 20, 7: 1, 8: {2: 1, 3: 1}, 9: 3},
            3: secret_code,
        }
    }
    return GeneRaTePk(str(CrEaTe_ProTo(fields).hex()), '0515', key, iv)
...
def ChEck_Limit_CLan(Uid, STaTus):
    data, max_use, file = (like_data_clan, 10, "BesTo_CLan_LiKes.json") if STaTus == "like" else (None, None, None)
...
def Remove_Uid(f, player_uid):
    try:
        with open(f, 'r+') as file: 
            ...
            file.seek(0)
            file.truncate()
            file.writelines(l for l in lines if l.strip() != player_uid)
        return True
...
# ================= NEW FUNCTIONS FOR TEAM LEADER EXTRACTION =================
def get_user_room_id(user_id, key, iv, client_socket):
    """
    Find the room/squad ID that the given user is currently in.
    Returns room_id (int) or None if not found.
    
    NOTE: This is a placeholder. You must implement actual protocol request/response parsing.
    For demonstration, it returns a dummy room_id for a specific user_id.
    """
    try:
        # Example: send GeT_Status packet and parse response to get room_id
        # You need to know the exact field numbers in the response.
        # Below is a dummy implementation:
        if str(user_id) == "123456789":
            return 987654321  # dummy room id
        else:
            # Try to get actual room id using GeT_Status
            packet = GeT_Status(user_id, key, iv)
            client_socket.send(packet)
            time.sleep(0.5)
            response = client_socket.recv(4096)
            # Parse response (you need to implement actual parsing)
            # For now, return None
            return None
    except Exception as e:
        print(f"get_user_room_id error: {e}")
        return None

def get_leader_from_packet(packet_hex):
    """
    Extract leader ID from a packet (response from server).
    Expected structure: the packet contains leader's user_id in field 5 -> 1 -> 8.
    Adjust field numbers according to your protocol.
    """
    try:
        json_str = DeCode_PackEt(packet_hex)
        if not json_str:
            return None
        data = json.loads(json_str)
        # Example path: data["5"]["data"]["1"]["data"]["8"]["data"]
        leader = data.get("5", {}).get("data", {}).get("1", {}).get("data", {}).get("8", {}).get("data", None)
        if leader:
            return leader
        # Alternative: try to find any field named "leader" or "8"
        return None
    except Exception as e:
        print(f"get_leader_from_packet error: {e}")
        return None

def get_room_leader(room_id, key, iv, client_socket):
    """
    Get the leader ID of a given room.
    Sends a packet (e.g., Join_Room or SquadInfo) and extracts leader from response.
    """
    try:
        # Send a packet that will return room info (including leader)
        # For example, Join_Room may return room details.
        packet = Join_Room(room_id, key, iv)
        client_socket.send(packet)
        time.sleep(0.5)
        response = client_socket.recv(4096)
        leader_id = get_leader_from_packet(response.hex())
        return leader_id
    except Exception as e:
        print(f"get_room_leader error: {e}")
        return None